 ...
 */
export function calculatePerformance(map, stars, mods, combo, misses, accuracy) {
  let multiplier = 1.1;
  if (mods & ojsama.modbits.nf) multiplier *= 0.90;
  if (mods & ojsama.modbits.hd) multiplier *= 1.10;
  const strainValue = calculateStrainPerformance(stars, mods, misses, accuracy / 100, combo);
  const accuracyValue = calculateAccuracyPerformance((difficltyRange(map.od, GREAT_MIN, GREAT_MID, GREAT_MAX) | 0), accuracy / 100, combo); // todo: clockRate, see DifficultyCalculator.cs:47, TaikoDifficultyCalculator.cs:65
...
export function calculateStrainPerformance(stars, mods, misses, accuracy, combo) {
